...
def brightness_loss(gen_img, real_img):
    gen_brightness = torch.mean(gen_img)
    real_brightness = torch.mean(real_img)
    return torch.abs(gen_brightness - real_brightness)
...
class BrightnessLoss(nn.Module):
    ...
    def forward(self, gen_img, real_img):
        """
        计算生成图像与真实图像的亮度损失
        :param gen_img: 生成图像 (B, C, H, W)，值域假设为 [-1, 1]
        :param real_img: 真实图像 (B, C, H, W)，值域假设为 [-1, 1]
        :return: 亮度损失值
        """
        # 将 [-1, 1] 范围的图像值映射到 [0, 1]（假设输入为归一化图像）
        gen_brightness = torch.mean((gen_img + 1.0) / 2.0)
        real_brightness = torch.mean((real_img + 1.0) / 2.0)
        
        # 计算生成图像与真实图像之间的亮度差异
        loss = torch.abs(gen_brightness - real_brightness)
        return loss