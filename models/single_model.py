...
class SingleModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        if opt.vgg > 0:   #这边是vgg网络
            ...
            self.vgg_loss.cuda()
            self.vgg = networks.load_vgg16(opt.vgg_pretrained_path, self.gpu_ids)
            self.vgg.eval()
    ...
    def backward_G(self, epoch):  #backward有forward的过程。。。。。,在这里不要使用ragan
        ...
        if self.opt.vgg > 0:
            ...
            self.loss_G = self.loss_G_A +  self.loss_fcn_b*vgg_w    #fcn_b是0 是自特征保持损失吗？
    ...
    def get_current_visuals(self):
        real_A = util.tensor2im(self.real_A.data)  #只返回第一张  256 256 3
        fake_B = util.tensor2im(self.fake_B.data)
